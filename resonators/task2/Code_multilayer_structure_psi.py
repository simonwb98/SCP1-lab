...
file = 'S1127_measurement_data_plus_model.txt'
...
def plot_data(input_file):
    ...
    plt.plot(energy, psi3, color[3], label=r'$\theta = 70^{\circ}$', linewidth=1)
    plt.plot(energy, psi4, color[0], label=r'$\theta = 75^{\circ}$', linewidth=1)
    ...
    plt.grid()
    lgd = plt.legend(ncol= 2, bbox_to_anchor=(0.8, -0.2), handleheight=2.0)#, loc='lower left')
    plt.title('Sample S1127 (Bragg Reflector)')
    # plt.xlim(0.0, 4.0)
    # plt.ylim(0.0, 16.0)
    # plt.xticks([1.0, 2.0, 3.0], [1.0, 2.0, 3.0])
    # plt.yticks([4, 8, 12, 16], [4, 8, 12, 16])
    plt.savefig('S1127', dpi=200, bbox_extra_artists=(lgd,), bbox_inches='tight')
