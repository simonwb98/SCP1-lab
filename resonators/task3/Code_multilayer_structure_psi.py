...
file = 'S1128_measurement_data_plus_fit.txt'
...
def plot_data(input_file):
    ...
    plt.grid()
    lgd = plt.legend(ncol= 2, loc= 'upper left', handleheight=2.0)#, loc='lower left')
    plt.title('Sample S1128 (Microresonator)')
    plt.xlim(2.0, 2.75)
    plt.ylim(-2, 80)
    # plt.xticks([1.0, 2.0, 3.0], [1.0, 2.0, 3.0])
    # plt.yticks([4, 8, 12, 16], [4, 8, 12, 16])
    plt.savefig('S1128', dpi=200, bbox_extra_artists=(lgd,), bbox_inches='tight')
