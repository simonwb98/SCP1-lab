...
def get_data(input_file):
    with open(input_file, 'r') as data_file:
        ...
        for line in data_file:
            ...
            model_psi1.append(float(h)) 
            model_delta1.append(float(i))
            model_psi2.append(float(j))
            model_delta2.append(float(k))
...
def plot_psi(input_file):
    ...
    plt.plot(energy, model_psi1, color[1], linestyle= '--', label=r'$\theta_{model} = 60^{\circ}$', linewidth=1)
    plt.plot(energy, model_psi2, color[2], linestyle='--', label=r'$\theta_{model} = 65^{\circ}$', linewidth=1)
    plt.plot(energy, model_psi3, color[0], linestyle='--', label=r'$\theta_{model} = 70^{\circ}$', linewidth=1)
    plt.xlabel(r'$h\nu (eV)$')
    plt.ylabel(r'$\Psi (^{\circ})$')
    plt.title('$YSZ$ on $Al_2O_3$ (' + name + ')')
    plt.grid()
