...
file = 'Task2_YSZonAl2O3_100nmFilm_simulation_Psi&Delta_vs_AOI@2eV.txt'
# name = file.strip('nm.txt')
...
def plot_data(input_file):
    ...
    ylabel('$\Delta (^{\circ})$')
    title(r'$YSZ$ layer, $d= 100$ nm - $\Psi$ and $\Delta$ at $E \cong 2$ eV')
    # for the legend, remember that we used two different axes so, we need 
    # to build the legend manually
    legend([line1, line2], [r'$\Psi$', r'$\Delta$'], loc='lower left')
    xlabel(r'$\theta (^{\circ})$')
    plt.grid()
    savefig('YSZ_fixed_energy', dpi=200)
    show()
